...
def SaveParentArtery(centerlines):
    ...
    for i in range(1, numberOfCells):  # cell0 is the one that goes to the aneurysm dome
        ...
        for j in range(cell.GetNumberOfPoints()):
            ...
            radiusArray.SetTuple1(count, centerlines.GetPointData().GetArray(
                radiusArrayName).GetTuple1(cell.GetPointId(j)))
...
def FindClippingPointOnParentArtery(centerlines, parentCenterlines, toll):
    ...
    # this is the cl that goes through the aneurysm
    centerlines.GetCellPoints(0, cell0PointIds)
...
def CreateParentArteryPatches(parentCenterlines, clipPoints):
    ...
    for i in range(0, numberOfCommonPatch):
        ...
        radiusArray.SetTuple1(i, parentCenterlines.GetPointData().GetArray(
    ...
    for j in range(numberOfDaughterPatches):
        ...
        for i in range(startId, cell.GetNumberOfPoints()):
            ...
            radiusArray.SetTuple1(count, parentCenterlines.GetPointData().GetArray(
...
def InterpolateTwoCells(startCell, endCell, numberOfSplinePoints, additionalPointId, additionalPoint):
    ...
    for i in range(numberOfSplinePoints):
        points.SetPoint(i, xspline.Evaluate(float(i)), yspline.Evaluate(
            float(i)), zspline.Evaluate(float(i)))
    return points
...
# OPTIONS TO SET
# automatically set to 1 for terminal aneurysms
useAdditionalInterpolationPoint = 0
# set to 1 if clipping points will be provided not computed from the centerlines
setClippingPoints = 0
# commonly 2.0; the DivergingTolerange for the identification of diverging points on each couple of centerlines is computed as (centerlineSpacing / DivergingRatioToSpacingTolerance)
...
# outputfilenames:
divergingPointsFilename = inputfiledirectory + \
    '/' + ID + '/' + ID + '_divergingpoints.vtp'
patchParentArteryFilename = inputfiledirectory + \
    '/' + ID + '/' + ID + '_patchcl.vtp'
interpolatedCenterlinesFilename = inputfiledirectory + \
    '/' + ID + '/' + ID + '_interpolatedcl.vtp'

if (aneurysmType == 'lateral'):

    if (setClippingPoints == 0):
        forwardCenterlinesFilename = inputfiledirectory + \
            '/' + ID + '/' + ID + '_forwardcl.vtp'
        backwardCenterlinesFilename = inputfiledirectory + \
            '/' + ID + '/' + ID + '_backwardcl.vtp'
        clippingPointsFilename = inputfiledirectory + \
            '/' + ID + '/' + ID + '_clippingpoints.vtp'
        ...
        centerlineSpacing = math.sqrt(vtk.vtkMath.Distance2BetweenPoints(
            parentArteryCenterlines.GetPoint(10), parentArteryCenterlines.GetPoint(11)))
        ...
        upstreamClippingPoint, upstreamDivergingPoint = FindClippingPointOnParentArtery(
            forwardCenterlines, parentArteryCenterlines, divergingTolerance)
        clippingPoints.InsertNextPoint(upstreamClippingPoint)
        divergingPoints.InsertNextPoint(upstreamDivergingPoint)

        downstreamClippingPoint, downstreamDivergingPoint = FindClippingPointOnParentArtery(
            backwardCenterlines, parentArteryCenterlines, divergingTolerance)
        ...
        parentArteryFileName = inputfiledirectory + \
            '/' + ID + '/' + ID + '_parentvessel.vtp'
        clippingPointsFileName = inputfiledirectory + \
            '/' + ID + '/' + ID + '_clippingpoints.vtp'
    ...
    patchParentArteryCenterlines = CreateParentArteryPatches(
        parentArteryCenterlines, clippingPoints)
    WritePolyData(patchParentArteryCenterlines, patchParentArteryFilename)

    print('Interpolating Patched Centerlines')
    interpolatedCenterlines = InterpolatePatchCenterlines(
        patchParentArteryCenterlines, parentArteryCenterlines)
...
if (aneurysmType == 'terminal'):
    ...
    if (setClippingPoints == 0):
        parentArteryCenterlinesFilename = inputfiledirectory + \
            '/' + ID + '/' + ID + '_parentvessel.vtp'
        daughter1CenterlinesFilename = inputfiledirectory + \
            '/' + ID + '/' + ID + '_dau1cl.vtp'
        daughter2CenterlinesFilename = inputfiledirectory + \
            '/' + ID + '/' + ID + '_dau2cl.vtp'
        clippingPointsFilename = inputfiledirectory + \
            '/' + ID + '/' + ID + '_clippingpoints.vtp'
        ...
        commonClippingPoint, commonDivergingPoint = FindClippingPointOnParentArtery(
            parentArteryCenterlines, parentArteryCenterlines, divergingTolerance)
        clippingPoints.InsertNextPoint(commonClippingPoint)
        divergingPoints.InsertNextPoint(commonDivergingPoint)

        dau1ClippingPoint, dau1DivergingPoint = FindClippingPointOnParentArtery(
            daughter1Centerlines, parentArteryCenterlines, divergingTolerance)
        clippingPoints.InsertNextPoint(dau1ClippingPoint)
        divergingPoints.InsertNextPoint(dau1DivergingPoint)

        dau2ClippingPoint, dau2DivergingPoint = FindClippingPointOnParentArtery(
            daughter2Centerlines, parentArteryCenterlines, divergingTolerance)
